...
<html>
	...
	<h1 style="text-align:center;">Welcome <?php echo 'New Student'; ?></h1><br><hr><br>
			...
			<div style="transform:translate(10%,0);width:90%;">
					...
					<?php
						if(isset($_POST["submit"]))
						{
							...
							{
								$sq="insert into admission(ADID,NAME,FNAME,DOB,GEN,PHO,MAIL,ADDR,SCLASS,SDOC) values({$_POST["adid"]},'{$_POST["name"]}','{$_POST["fname"]}','{$edate}','{$_POST["gen"]}','{$_POST["pho"]}','{$_POST["email"]}','{$_POST["addr"]}','{$_POST["cla"]}','{$target_file}')";
								...
								{
									echo "<div class='error'>Applied Failed</div>";
								}
							}
				...
				<div class="lbox" style="width:45%;">
					<label>Admission ID</label><br>
						<?php
							$no="101";
							$sql="select * from admission order by ADID desc limit 1";
							$res=$db->query($sql);
							if($res->num_rows>0)
							{
						...
						?>
					<input type="number" class="input3" name="adid" style="background:#b1b1b1;" value="<?php echo $no;?>" readonly  ><br><br>
				...
				<label> Parent's Mail Id</label><br>
					...
					<br><br>
					<label>Add Documents</label><br>
					<input type="file"  class="input3" multiple required name="img"><br><br>
			
			<button type="submit" class="btn" name="submit">Apply For Admission</button>
				</div>
