...
<html>
	...
	<?php include"slider1.php";?>
		...
		<div class="login">
			<h1 class="heading" style="margin:2%;">Class Management's Login</h1>
			<div class="log" style="margin:auto;">
				<?php
					if(isset($_POST["login"]))
					{
						$sql="select * from class_management where CMNAME='{$_POST["name"]}'and CMPASS='{$_POST["pass"]}'";
						$res=$db->query($sql);
						if($res->num_rows>0)
						{
							$ro=$res->fetch_assoc();
							$_SESSION["CMID"]=$ro["CMID"];
							$_SESSION["CMNAME"]=$ro["CMNAME"];
							echo "<script>window.open('class_management_home.php','_self');</script>";
						}
						else
						{
