			...
			<div id="section">
				...
				<div class="content1">
					...
					<div class="lbox1">	
						...
						<?php 
							$c="select CLA,SUB from hclass where TID={$_SESSION["TID"]}";
							$c=$db->query($c);
							if($c->num_rows>0)
							{
								...
								{
									$sl="select distinct(EDATE) FROM exam where CLASS='{$co["CLA"]}' and SUB='{$co["SUB"]}'";
									$r=$db->query($sl);
										if($r->num_rows>0)
											{
						...
						<?php
							if(isset($_POST["view"]))
							{
								...
								if($c->num_rows>0)
								{
									while($co=$c->fetch_assoc())
									{
										$sql="select * FROM exam where EDATE='{$_POST["edate"]}' and CLASS='{$co["CLA"]}' and SUB='{$co["SUB"]}'";
										$re=$db->query($sql);
										if($re->num_rows>0)
										{
											echo '
