			...
			<div id="section">
				...
				<h3 >View Notices </h3><br>
					...
					<select name="notice_date" required class="input3">
						...
						<?php 
							 $sl="select distinct(notice_date) FROM notice where (notice_to='fee' or notice_to='all')";
							$r=$db->query($sl);
								if($r->num_rows>0)
									{
					...
					<div class="Output">
						<?php
							if(isset($_POST["view"]))
							{
								...
								$sql="select * from notice where (notice_date='{$_POST["notice_date"]}' AND (notice_to='fee' or notice_to='all'))";
								$re=$db->query($sql);
								if($re->num_rows>0)
								{
									echo '
