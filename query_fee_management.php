			...
			<div id="section">
				...
				<h3 >View Queries </h3><br>
					...
					<select name="query_no" required class="input3">
						...
						<?php 
							 $sl="select * FROM queries where status=0 and query_to='fee'";
							$r=$db->query($sl);
								if($r->num_rows>0)
									{
					...
					<?php
						...
						{
							$sq="update queries set response='{$_POST["respo"]}',status=1 where (query_no={$_POST["queryno"]} and query_to='fee') ";
					...
					<form method="post" action="<?php echo $_SERVER["PHP_SELF"];?>">
						<label>Query NO.</label><br>
					     <input type="number" name="queryno" required class="input">
					     <br><br>
