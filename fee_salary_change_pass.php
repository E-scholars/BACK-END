<?php
	...
	{
		echo"<script>window.open('fee_mangement_home.php?mes=Access Denied...','_self');</script>";
		
	}	
	
	
	$sql="SELECT * FROM fee_management WHERE FID={$_SESSION["FID"]}";
		$res=$db->query($sql);
			...
			<div id="section">
				...
				<div class="content">
					...
					<div class="lbox1">	
						...
						{
							$sql="select * from fee_management where FPASS='{$_POST["opass"]}' and FID='{$_SESSION["FID"]}'";
							$result=$db->query($sql);
								if($result->num_rows>0)
								{
									if($_POST["npass"]==$_POST["cpass"])
									{
										$sql="UPDATE fee_management SET  FPASS='{$_POST["npass"]}' where  FID='{$_SESSION["FID"]}'";
										$db->query($sql);
