...
<html>
	...
	<?php include"slider1.php";?>
		...
		<div class="login">
			<h1 class="heading" style="margin:2%;">Fee Salary Management's Login</h1>
			<div class="log" style="margin:auto;">
				<?php
					if(isset($_POST["login"]))
					{
						$sql="select * from fee_management where FNAME='{$_POST["name"]}'and FPASS='{$_POST["pass"]}'";
						$res=$db->query($sql);
						if($res->num_rows>0)
						{
							$ro=$res->fetch_assoc();
							...
							echo "<script>window.open('fee_management_home.php','_self');</script>";
						}
						else
						{
