			...
			<div class="content">
				...
				<div class="lbox1">	
					...
					<select name="cla" required class="input3">
						...
						<?php 
							 $sl="select DISTINCT(CLA) FROM hclass where TID= {$_SESSION["TID"]}";
							$r=$db->query($sl);
								if($r->num_rows>0)
									{
				...
				<div class="rbox">
					<label>Section</label><br>
						...
						<?php 
							 $sql="SELECT DISTINCT(SEC) FROM hclass where TID= {$_SESSION["TID"]}";
							$re=$db->query($sql);
								if($re->num_rows>0)
									{
										echo"<option value=''>Select</option>";
										...
										{
											echo "<option value='{$r["SEC"]}'>{$r["SEC"]}</option>";
										}
									}
						?>
				...
				<div class="Output">
						<?php
							if(isset($_POST["view"]))
							{
								...
								$tmp="select TID from hclass where CLA='{$_POST["cla"]}' and SEC='{$_POST["sec"]}' and TID={$_SESSION["TID"]}";
								$tm=$db->query($tmp);
								if($tm->num_rows>0)
								{
